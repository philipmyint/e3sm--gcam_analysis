...
def extract_dataframe_rows_for_given_year(df, year):
    """ 
    Processes the Pandas DataFrame so that it contains only the rows pertaining to the given year. Also changes the 'time' column to 'Year'.
    ...
    Returns:
        DataFrame containing only the rows for the given year, with the 'time' column changed to 'Year'.
    """
    df.index.names = ['Year' if name == 'time' else name for name in df.index.names]
    df = df.reset_index(['Year'])
    df = df[df['Year'] == year].drop(columns=['Year'])
    return df

def extract_netcdf_file_into_dataframe_single_year(file, variables, year):
    """ 
    ...
    areas, ds, _, _ = find_gridcell_areas_in_netcdf_file(file)
    ...
    # Convert the Dataset to create an overall DataFrame that stores all variables except those for PFTs. Create a second DataFrame to store PFTs. 
    ...
    if variables_except_pfts:
        ...
        # Reduce the size of the DataFrame to only the rows for the specific year of interest.
    ...
    if 'PCT_NATVEG' in variables:
        df['PCT_NATVEG'] = df['PCT_NATVEG'].groupby(['lsmlat', 'lsmlon']).mean().fillna(0)/100
    ...
    if 'PCT_NAT_PFT' in variables:
        ...
        for index, pft_label in enumerate(pft_labels):
            ...
            df_this_pft = df_pfts[(df_pfts['natpft'] >= pft_min_index) & 
                                                (df_pfts['natpft'] <= pft_max_index)]
            # Reduce the size of the DataFrame to only the rows for the specific year of interest.
            df_this_pft = extract_dataframe_rows_for_given_year(df_this_pft, year)
    ...
    # Convert the grazing and harvest variables into an area.
    grazing_harvest_variables = ['GRAZING', 'HARVEST_SH1', 'HARVEST_SH2', 'HARVEST_SH3', 'HARVEST_VH1', 'HARVEST_VH2']
    pft_multipliers = ['GRASS_AREA (km^2)', 'FOREST_AREA (km^2)', 'FOREST_AREA (km^2)', 'FOREST_AREA (km^2)', 'FOREST_AREA (km^2)', 
                          'FOREST_AREA (km^2)']
    for index in range(len(grazing_harvest_variables)):
        grazing_harvest_variable = grazing_harvest_variables[index]
        pft_multiplier = pft_multipliers[index]
        df[grazing_harvest_variable] = df[grazing_harvest_variable]*df['AREA (km^2)']*df['FRAC_VEG']*df[pft_multiplier]
...
def extract_time_series_from_netcdf_file(inputs):
    ...
    """
    # Extract all user-selected choices. Calculate the total number of years. Set the path and name of the file.
    ...
    arguments = list(zip([file]*num_years, [variables]*num_years, range(start_year, end_year+1)))
