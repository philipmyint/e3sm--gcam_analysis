...
def plot_spatial_data_from_netcdf_files(inputs):
    ...
    if num_file_sets == 2:
        ...
        if plot_type_for_2_sets == 'percent_difference':
            # Plot percent differences between the two data sets. If we have a divide-by-zero error, replace the resulting NaN with 0.
            df = (df_test_set - df_control_set).fillna(0)*100
            da = df.to_xarray()
